...
#Region " Fields"
        ...
        Private sWorkOrder, sCopeNumber, sDragNumber, sPourRate, sWhlType, sCHONO, sDHONO, sOPERATOR1, sSIC, sSHIFTID
...
#Region " Methods"
        ...
        Public Shared Function PouringStartTime(ByVal HeatNumber As Long) As DataTable
            ...
            Try
                da.SelectCommand.CommandText = " select PourStartTime , PourStartTemp from mm_pouring_StartTime  " & _
                    " where heat_number = @HeatNumber "
    ...
    Public Class MLDING
        Public Shared Function GetNFHeatWheels(ByVal HeatNo As Long) As DataTable
            ...
            Try
                ...
                da.SelectCommand.CommandText = " select row_number() over ( order by TimeIn ,TimeOut ) Sl , " & _
                    " convert(varchar(10),TimeInDate,103) ChargeDt , TimeInSh Sh , WheelType , HeatNo , " & _
                    " WheelNo , SIC , OP1 , OP2 , PedNo , PedPosition Pos , " & _
                    " convert(varchar(10),TimeIn,103) DateIn , convert(varchar(5),TimeIn,108) TimeIn , " & _
                    " convert(varchar(10),TimeOut,103) DateOut , convert(varchar(5),TimeOut,108) TimeOut , " & _
                    " Quencher , Remarks , SlNo" & _
        ...
        Public Shared Function GetMRXCReasons(ByVal MRXC As String) As DataTable
            ...
            Try
                ...
                da.SelectCommand.CommandText = " select MRXC , Reasons , XCId " &
        ...
        Public Shared Function GetMRXCWheelDetails(ByVal TappedDate As Date) As DataTable
            ...
            Try
                ...
                da.SelectCommand.CommandText = " select rtrim(status) MRXC ,  b.wheel_number Whl  , " &
                    " a.heat_number  Heat , pour_order PO , rtrim(description) WhlType , Reasons , d.Remarks , " &
                    " LIC , SIC from mm_vw_HeatTapped a inner join mm_wheel b" &
                    " on a.heat_number = b.heat_number inner join mm_pouring c" &
                    " on b.heat_number = c.heat_number and b.wheel_number = c.engraved_number" &
                    " and status = rejection_code left outer join mm_vw_MRXCDetails d " &
                    " on b.heat_number = d.HeatNo and b.wheel_number = d.WheelNo " &
                    " where TappedDate = @TappedDate and location = 'mopo'" &
        ...
        Public Shared Function GetWheelData(ByVal Wh As Integer, ByVal Ht As Long, ByVal Dt As Date, ByVal Sh As String) As DataTable
            ...
            Dim da As SqlClient.SqlDataAdapter = rwfGen.Connection.Adapter
            Try
                da.SelectCommand.CommandText = "select * from mm_normalizing_furnace_loading " & _
                    " where loading_date = @Dt and shift_code = @Sh" & _
        ...
        Public Shared Function CheckWheel(ByVal Wh As Integer, ByVal Ht As Long) As Boolean
            ...
            Try
                If oCmd.Connection.State = ConnectionState.Closed Then oCmd.Connection.Open()
                oCmd.CommandText = "select @cnt = count(*) from mm_wheel " & _
        ...
        Public Shared Function ShopCodes(ByVal sUser As String) As DataSet
            ...
            Dim da As SqlClient.SqlDataAdapter = rwfGen.Connection.Adapter
            Try
                da.SelectCommand.CommandText = "select shop_code ,Group_code  from  mm_ShopGround_ShopCode ;" & _
                    " select rtrim(A.stn_id)+'-'+rtrim(a.stn_desc) StnDesc, a.stn_id  from  mm_mmsdt_dump a inner join  mm_ShopGround_ShopCode b " & _
        ...
        Public Shared Function MROutTurnData(ByVal FrDt As Date, ByVal ToDt As Date) As DataTable
            ...
            Try
                da.SelectCommand.CommandText = "select convert(varchar(11),OutTurnDate,103) OutTurnDate  , Shift , sum(WhlsCast) WhlsCast ,  " &
                    "sum(MROffLoad) MROffLoad ,  " &
                    "sum(SGM1Qty) SGM1Qty, sum(SGM2Qty) SGM2Qty, sum(SGM3Qty) SGM3Qty ,  " &
                    "sum(SGM4Qty) SGM4Qty, sum(HCM1Qty) HCM1Qty , sum(HCM2Qty) HCM2Qty, sum(HCM3Qty) HCM3Qty,  " &
                    "sum(CBQty) CBQty, sum(CSQty) CSQty , sum(DSQty) DSQty,  " &
                    "sum(COLQty) COLQty , sum(DOLQty) DOLQty , sum(SWCQty) SWCQty , sum(SWPGQty) SWPGQty , " &
                    "sum(NBDQty) NBDQty , sum(DDUQty) DDUQty , " &
                    "sum(CopesQty) CopesQty , sum(DragsQty) DragsQty , " &
                    "sum(CopesMQty) CopesMQty , sum(DragsMQty) DragsMQty , " &
                    "sum(SandQty) SandQty , sum(PTQty) PTQty , " &
                    "max(case when MPO1 = '1' then 'Yes' else 'No' end) MPO1 , " &
                    "max(case when MPO2 = '1' then 'Yes' else 'No' end) MPO2 , " &
                    "max(case when MPO3 = '1' then 'Yes' else 'No' end) MPO3  " &
                    "from mm_MROutTurn " &
                    ...
                    "union " &
                    "select 'Total' OutTurnDate , 'All' Shift , sum(WhlsCast) WhlsCast , " &
                    "sum(MROffLoad) MROffLoad ,  " &
                    "sum(SGM1Qty) SGM1Qty, sum(SGM2Qty) SGM2Qty, sum(SGM3Qty) SGM3Qty ,  " &
                    "sum(SGM4Qty) SGM4Qty, sum(HCM1Qty) HCM1Qty , sum(HCM2Qty) HCM2Qty, sum(HCM3Qty) HCM3Qty,  " &
                    "sum(CBQty) CBQty, sum(CSQty) CSQty , sum(DSQty) DSQty,  " &
        ...
        Public Shared Function MROutTurn(ByVal OutTurnDate As Date, ByVal Shift As String) As DataTable
            ...
            Try
                ...
                "sum(SGM4Qty) SGM4Qty, sum(HCM1Qty) HCM1Qty , sum(HCM2Qty) HCM2Qty, sum(HCM3Qty) HCM3Qty,  " &
                "sum(CBQty) CBQty, sum(CSQty) CSQty , sum(DSQty) DSQty,  " &
                "sum(COLQty) COLQty , sum(DOLQty) DOLQty , sum(SWCQty) SWCQty , sum(SWPGQty) SWPGCQty , " &
                "sum(NBDQty) NBDQty , sum(DDUQty) DDUQty , " &
                "sum(CopesQty) CtoL , sum(DragsQty) DtoL , " &
                "sum(CopesMQty) CopesMQty , sum(DragsMQty) DragsMQty , " &
                "sum(SandQty) Sand , sum(PTQty) PTubes ," &
                "max(case when MPO1 = '1' then 'Yes' else 'No' end) MPO1 , " &
                "max(case when MPO2 = '1' then 'Yes' else 'No' end) MPO2 , " &
                "max(case when MPO3 = '1' then 'Yes' else 'No' end) MPO3  " &
                "from mm_MROutTurn " &
                ...
                "sum(SGM4Qty) SGM4Qty, sum(HCM1Qty) HCM1Qty , sum(HCM2Qty) HCM2Qty, sum(HCM3Qty) HCM3Qty,  " &
                "sum(CBQty) CBQty, sum(CSQty) CSQty , sum(DSQty) DSQty,  " &
                "sum(COLQty)/3 COLQty , sum(DOLQty)/3 DOLQty ,  sum(SWCQty) SWCQty , sum(SWPGQty) SWPGCQty ,  " &
                "sum(NBDQty) NBDQty , sum(DDUQty) DDUQty , " &
                "sum(CopesQty) CtoL , sum(DragsQty) DtoL , " &
                "sum(CopesMQty) CopesMQty , sum(DragsMQty) DragsMQty , " &
                "sum(SandQty) Sand , sum(PTQty) PTubes , '' ,  '' ,  '' " &
                ...
                "group by OutTurnDate  " &
                "union " &
                "select 'Total' OutTurnDate , 'All' Shift , sum(WhlsCast) WhlsCast ,sum(MROffLoad) MROffLoad ,  " &
                "sum(SGM1Qty) SGM1Qty, sum(SGM2Qty) SGM2Qty, sum(SGM3Qty) SGM3Qty ,  " &
                "sum(SGM4Qty) SGM4Qty, sum(HCM1Qty) HCM1Qty , sum(HCM2Qty) HCM2Qty, sum(HCM3Qty) HCM3Qty,  " &
                "sum(CBQty) CBQty, sum(CSQty) CSQty , sum(DSQty) DSQty,  " &
                "sum(COLQty)/count(*) COLQty , sum(DOLQty)/count(*) DOLQty ,  sum(SWCQty) SWCQty , sum(SWPGQty) SWPGCQty ,  " &
                "sum(NBDQty) NBDQty , sum(DDUQty) DDUQty , " &
                "sum(CopesQty) CtoL , sum(DragsQty) DtoL , " &
                "sum(CopesMQty) CopesMQty , sum(DragsMQty) DragsMQty , " &
                "sum(SandQty) Sand , sum(PTQty) PTubes , '' , '' , '' " &
        ...
        Public Function SaveMouldTran(ByVal TransDt As Date, ByVal Sh As String, ByVal LineJE As String, ByVal ShiftIC As String, ByVal MldType As Integer, ByVal MpoList As String, ByVal MouldNo As String, ByVal Defect As Integer, ByVal TransTime As DateTime, ByVal AddAfterMouldNo As String) As Boolean
            ...
            Try
                If oCmd.Connection.State = ConnectionState.Closed Then oCmd.Connection.Open()
                oCmd.CommandText = "insertinto mm_MouldTransactionNew " &
        ...
        Public Function OutTurn(ByVal OutTurnDate As Date, ByVal Shift As String, ByVal ShiftInC As String, ByVal SGM1Qty As Integer, ByVal SGM1InC As String, ByVal SGM1Remarks As String, ByVal SGM2Qty As Integer, ByVal SGM2InC As String, ByVal SGM2Remarks As String, ByVal SGM3Qty As Integer, ByVal SGM3InC As String, ByVal SGM3Remarks As String, ByVal SGM4Qty As Integer, ByVal SGM4InC As String, ByVal SGM4REmarks As String, ByVal HCM1Qty As Integer, ByVal HCM1InC As String, ByVal HCM1Remarks As String, ByVal HCM2Qty As Integer, ByVal HCM2InC As String, ByVal HCM2Remarks As String, ByVal HCM3Qty As Integer, ByVal HCM3InC As String, ByVal HCM3Remarks As String, ByVal CBQty As Integer, ByVal CBInC As String, ByVal CBRemarks As String, ByVal CSQty As Integer, ByVal CSInC As String, ByVal CSRemarks As String, ByVal DSQty As Integer, ByVal DSInC As String, ByVal DSRemarks As String, ByVal COLQty As Integer, ByVal COLInC As String, ByVal COLRemarks As String, ByVal DOLQty As Integer, ByVal DOLInC As String, ByVal DOLRemarks As String, ByVal WhlsCast As Integer, ByVal MROffLoad As Integer, ByVal SWLess400 As Integer, ByVal HCLess250 As Integer, ByVal CSGr230 As Integer, ByVal CSLess170 As Integer, ByVal DSGr230 As Integer, ByVal DSLess170 As Integer, ByVal SWRecSts As String, ByVal HCRecSts As String, ByVal CSRecSts As String, ByVal DSRecSts As String, ByVal C5IE As String, ByVal C5IW As String, ByVal C5KE As String, ByVal C5KW As String, ByVal SWCQty As Integer, ByVal SWCInC As String, ByVal SWCRemarks As String, ByVal SWPGQty As Integer, ByVal SWPGInC As String, ByVal SWPGRemarks As String, ByVal NBDQty As Integer, ByVal NBDInC As String, ByVal NBDRemarks As String, ByVal DDUQty As Integer, ByVal DDUInC As String, ByVal DDURemarks As String, ByVal CopesQty As Integer, ByVal CopesInC As String, ByVal CopesRemarks As String, ByVal DragsQty As Integer, ByVal DragsInC As String, ByVal DragsRemarks As String, ByVal CopesMQty As Integer, ByVal CopesMInC As String, ByVal CopesMRemarks As String, ByVal DragsMQty As Integer, ByVal DragsMInC As String, ByVal DragsMRemarks As String, ByVal SandQty As Integer, ByVal SandInC As String, ByVal SandRemarks As String, ByVal PTQty As Integer, ByVal PTInC As String, ByVal PTRemarks As String, ByVal MPO1 As String, ByVal MPO2 As String, ByVal MPO3 As String) As Boolean
            Dim blnInsert As Boolean
            Dim ds As New DataSet()
            Try
                ds = RWF.MLDING.CheckOutTurn(CDate(OutTurnDate), Shift)
            ...
            Try
                ...
                strInsert = " insert into mm_MROutTurn ( OutTurnDate,Shift,WhlsCast , MROffLoad ," &
                            " SGM1Qty," &
                            ...
                            " HCRecSts , CSRecSts , DSRecSts , C5IE , C5IW , C5KE , C5KW ,  " &
                            ...
                            " values ( @OutTurnDate,@Shift,@WhlsCast , @MROffLoad, " &
                            " @SGM1Qty," &
                            " @SGM1InC,@SGM1Remarks,@SGM2Qty,@SGM2InC,@SGM2Remarks,@SGM3Qty,@SGM3InC,@SGM3Remarks," &
                            " @SGM4Qty,@SGM4InC,@SGM4REmarks,@HCM1Qty,@HCM1InC,@HCM1Remarks,@HCM2Qty,@HCM2InC," &
                            " @HCM2Remarks,@HCM3Qty,@HCM3InC,@HCM3Remarks,@CBQty, @CBInC, @CBRemarks," &
                ...
                strUpdate = " update mm_MROutTurn set WhlsCast = @WhlsCast ,  MROffLoad = @MROffLoad , " &
                            " SGM1Qty   = @SGM1Qty  ,  SGM1InC   = @SGM1InC ,  SGM1Remarks   = @SGM1Remarks , " &
