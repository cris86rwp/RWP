Namespace RWF
    <Serializable()> Public Class tables
        ...
        Public Shared Function CheckPourTime(ByVal HeatNumber As Long, ByVal PourOrder As Integer, ByVal PourTime As Date) As Integer
            ...
            oCmd.Parameters.Add("@cnt", SqlDbType.Int, 4).Direction = ParameterDirection.Output
            oCmd.CommandText = "if @PourTime BETWEEN isnull((SELECT Top 1 pour_time FROM mm_pouring " & _
                    " WHERE heat_number = @HeatNumber and pouring_order < @PourOrder ORDER BY pouring_order DESC),'1/1/1900')  " & _
                    " AND isnull((SELECT Top 1 DateAdd(d,1,pour_time) FROM mm_pouring  " & _
                    " WHERE heat_number = @HeatNumber  and pouring_order > @PourOrder  ORDER BY pouring_order),GetDate()) " & _
