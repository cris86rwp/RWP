...
#Region " Tables"
            ...
            Try
                ...
                If ExtendedEdit = False Then
                    If Group = "WHLMLT" Then
                        da.SelectCommand.CommandText = "select  breakdown_from_time BDFromTime , machine_code MCcode,  memo_number MemoNum, " &
                            " memo_slip_number MemoSlipNum ,  remarks Remarks , operator , production_affected ProdAffec " &
                            " , breakdown_to_time , CodeType from ms_vw_breakdown_memo_WHLMLT where shop_code = @Shop " &
                            " and maintenance_group = @Group and breakdown_to_time = '1900-01-01 00:00:00.000' " &
                            " order by breakdown_from_time , machine_code "
                    Else
                        da.SelectCommand.CommandText = "select  breakdown_from_time BDFromTime , machine_code MCcode,  memo_number MemoNum, " &
                            " memo_slip_number MemoSlipNum ,  remarks Remarks , operator , production_affected ProdAffec " &
                            " , breakdown_to_time , CodeType from ms_vw_breakdown_memo where shop_code = @Shop " &
                            " and maintenance_group = @Group and breakdown_to_time = '1900-01-01 00:00:00.000' " &
                            " order by breakdown_from_time , machine_code "
                    End If
                Else
                    If Group = "WHLMLT" Then
                        da.SelectCommand.CommandText = "select  a.breakdown_from_time BDFromTime, a.machine_code MCcode,  a.memo_number MemoNum, " &
                            " a.memo_slip_number MemoSlipNum , a.remarks Remarks , a.operator , a.production_affected ProdAffec " &
                            " , a.breakdown_to_time , CodeType from ms_vw_breakdown_memo_WHLMLT a " &
                            " inner join mm_vw_EditableBreakdownsTillPhlGen b on a.maintenance_group = b.maintenance_group and a.memo_number = b.memo_number " &
                            " where a.shop_code = @Shop and a.breakdown_to_time = '1900-01-01 00:00:00.000' " &
                            " and a.maintenance_group = @Group " &
                            " order by a.breakdown_from_time , a.machine_code "
                    Else
                        da.SelectCommand.CommandText = "select  a.breakdown_from_time BDFromTime, a.machine_code MCcode,  a.memo_number MemoNum, " &
                            " a.memo_slip_number MemoSlipNum , a.remarks Remarks , a.operator , a.production_affected ProdAffec " &
                            " , a.breakdown_to_time , CodeType from ms_vw_breakdown_memo a " &
                            " inner join mm_vw_EditableBreakdownsTillPhlGen b on a.maintenance_group = b.maintenance_group and a.memo_number = b.memo_number " &
                            " where a.shop_code = @Shop " &
                            " and a.maintenance_group = @Group " &
                            " order by a.breakdown_from_time , a.machine_code "
                    End If
                End If
        ...
        Public Shared Function MemoNumber(ByVal Group As String, ByVal Memoslip As String, ByVal MemoNo As Integer) As Integer
            ...
            Try
                ...
                oCmd.CommandText = "select @Cnt = count(*) from ms_breakdown_memo " &
        ...
        Public Shared Function MemoSlipNumber(ByVal Group As String, ByVal Memoslip As String) As Integer
            ...
            Try
                If oCmd.Connection.State = ConnectionState.Closed Then oCmd.Connection.Open()
                oCmd.CommandText = "select @MemoNo = count(*) from ms_breakdown_memoslips " &
        ...
        Public Shared Function GetBDDetails(ByVal Group As String, ByVal BDDate As Date, ByVal Shift As String) As DataTable
            ...
            Try
                ...
                da.SelectCommand.CommandText = " select row_number() over ( order by memo_number ) SlNo , " &
                    " memo_number MemoNo , memo_slip_number MemoSlipNo , " &
                    " machine_code Machine , MachineShortName , " &
                    " convert(varchar(11),breakdown_from_time,103) BDFrDate , " &
                    " convert(varchar(5),breakdown_from_time,108) BDFrTime ," &
                    " convert(varchar(11),breakdown_to_time,103) BDToDate , " &
                    " convert(varchar(5),breakdown_to_time,108) BDToTime , TimeLoss ,  " &
                    " case when production_affected = 1 then 'Yes' else 'No' end YorN , CodeType , " &
                    " breakdown_code BDCode, BreakdownCode" &
                    " from ms_vw_breakdown_memo " &
        ...
        Public Shared Function GetBDMemos(ByVal Group As String, ByVal MemoDate As Date, ByVal Shift As String) As DataTable
            ...
            Try
                ...
                da.SelectCommand.CommandText = " select distinct a.code_type , a.description from code_type a " &
                    " inner join code b  on a.application = b.application and a.code_type = b.code_type " &
                    " and substring(code,1,3) = @EqpID " &
            ...
            Try
                If Group.Trim = "WHLMLT" AndAlso EqpID = "ALL" AndAlso (BDCode = "BDC" Or BDCode = "BDMR" Or BDCode = "BDN" Or BDCode = "BDP" Or BDCode = "BDU" Or BDCode = "BDRT") Then
                    da.SelectCommand.CommandText = "SELECT  codeDesc , code from mm_vw_BDCodesNew " &
                         ...
                         " ORDER BY codeDesc  "
                Else
                    da.SelectCommand.CommandText = "SELECT  codeDesc , code from mm_vw_BDCodes " &
                         " where BDType = @BDCode and Eqp = @EqpID " &
                         " ORDER BY codeDesc  "
                End If
        ...
        Public Shared Function GetBDCodes(ByVal EqpID As String, Optional ByVal Group As String = "") As DataTable
            ...
            Try
                ...
                If Group.Trim = "WHLMLT" Then
                    da.SelectCommand.CommandText = " select BDType , BDTypeDesc from ms_BDType where BDType!='BDRT'"
                    da.Fill(ds)
                Else
                    da.SelectCommand.CommandText = " select BDType , BDTypeDesc from mm_vw_BDTypes " &
                        " where Eqp = @EqpID "
                    da.Fill(ds)
                End If
                If ds.Tables(0).Rows.Count = 0 Then
            ...
            End Try
        End Function
        Public Shared Function GetBDCodes1(ByVal EqpID As String, Optional ByVal Group As String = "") As DataTable
            Dim da As SqlClient.SqlDataAdapter = rwfGen.Connection.Adapter
            Dim ds As New DataSet()
            Try
                da.SelectCommand.Parameters.Add("@EqpID", SqlDbType.VarChar, 50).Value = EqpID
        ...
        Public Shared Function GetMemoNumber(ByVal Group As String) As Integer
            ...
            Try
                If oCmd.Connection.State = ConnectionState.Closed Then oCmd.Connection.Open()
                oCmd.CommandText = "select @MemoNo = max(memo_number) from ms_breakdown_memo " &
        ...
        Public Shared Function GetMachineCodes(ByVal Group As String, ByVal Shop As String) As DataTable
            ...
            Try
                If Shop.Trim.ToLower = "rt" Then
                    da.SelectCommand.CommandText = "select (substring(machine_code,3,10)+'--'+ description) mccode_desc , " &
                        " rtrim(machine_code) machine_code " &
                        " from ms_machinery_master where location_code = @Shop " &
                        " and equipment_code in  ( select equipment_id from ms_group_location " &
                        " where location = @Shop and group_code = @Group )  order by  description "
                Else
                    da.SelectCommand.CommandText = "select rtrim(description) mccode_desc , " &
                       " rtrim(machine_code) machine_code " &
                       " from ms_machinery_master where location_code = @Shop " &
                       " and equipment_code in  ( select equipment_id from ms_group_location " &
                       " where location = @Shop and group_code = @Group )  " &
                       " and machine_code not in ( select machine_code  from ms_MachinesNotInUse )  " &
...
#Region " Methods"
        ...
        Public Function SaveMemo() As Boolean
            ...
            Try
                ...
                If oCmd.ExecuteNonQuery = 1 Then
                    oCmd.CommandText = "insert into ms_breakdown_details( maintenance_group , memo_number , breakdown_code ) " &
                        " values( @Group , @MemoNo , @BDCode )"
                    If oCmd.ExecuteNonQuery = 1 Then
                        oCmd.CommandText = " insert into ms_breakdown_memo ( maintenance_group , memo_number , memo_slip_number , " &
                          " breakdown_from_time , remarks , breakdown_date , shift_code , shop_code , machine_code , " &
                          " status , operator , production_affected , breakdown_type ) values " &
                          " ( @Group , @MemoNo , @MemoSlip , @BDFromTime , @Remarks , @BDDate , @Shift , @Shop , @Machine , " &
        ...
        Public Function UpdateMemo() As Boolean
            ...
            oCmd.CommandText = "update ms_breakdown_memo set  breakdown_to_time = @breakdown_to_time , " &
                   " remarks = @remarks , production_affected = @production_affected " &
