Namespace PCO
    Public Class tables
        ...
        Public Shared Function GetWOShops() As DataTable
            Dim da As SqlClient.SqlDataAdapter = rwfGen.Connection.Adapter
            Dim ds As New DataSet()
            da.SelectCommand.CommandText = "select distinct Shop from mm_vw_WorkorderShops "
            Try
                da.Fill(ds)
        ...
        Public Shared Function RWFProductList(ByVal Product As String) As DataSet
            ...
            Select Case Product
                ...
                Case "WheelSet"
                    ProductCode = "[5,6]%"
            End Select
            da.SelectCommand.CommandText = "Select product_code ProdCode , " & _
                " case when isnull(type,'') = '' then 'RWF' else rtrim(type) end Make ," & _
                " rtrim(description) ProductCodeShortDescription , rtrim(drawing_number) ProductDrawingNumber" & _
                " FROM mm_product_master " & _
                " where product_code in ( " & _
                " select distinct a.product_code" & _
                " FROM mm_product_master a " & _
                " inner join mm_workorder b" & _
                " on a.product_code = b.product_code" & _
                " where a.product_code like @ProductCode ) " & _
                " and product_code not in ( select ProductCode from mm_product_UnifiedPL ) " & _
                " order by product_code ; " & _
                " select ProductCode ,  a.UnifiedPL , drawing_number , " & _
                " ItemDescription , SpecificationNo, DrawingNo, Category" & _
                " from mm_product_UnifiedPL a inner join mm_product_UnifiedPL_Details b" & _
                " on a.UnifiedPL = b.UnifiedPL inner join mm_product_master c  " & _
                " on a.ProductCode = c.Product_Code  " & _
...
#Enable Warning BC42104 ' Variable is used before it has been assigned a value
        ...
        Public Shared Function AuthorisedUserIDPassword(ByVal User As String) As String
            Dim cmd As SqlClient.SqlCommand = rwfGen.Connection.CmdObj
            cmd.CommandText = "SELECT @password = password FROM mis.dbo.menu_your_password WHERE employee_code = @User and application = 'MMS' and group_code = 'PCOPCO' "
        ...
        Public Shared Function getConsigneeDetails() As DataTable
            ...
            Try
                ...
                da.SelectCommand.CommandText = "SELECT code , name , address , city , pincode , passing_authority , customer_code , " &
                    " ShortName , consignee_gst FROM mm_customer_consignee "
...
#Enable Warning BC42104 ' Variable is used before it has been assigned a value
        ...
        Public Shared Function GetProducts() As DataTable
            ...
            Dim da As SqlClient.SqlDataAdapter = rwfGen.Connection.Adapter
            Try
                da.SelectCommand.CommandText = "SELECT product_code + ' : ' + description as text, product_code as value FROM mm_product_master order by Product_code"
                da.Fill(ds)
                Return ds.Tables(0).Copy
            Catch exp As Exception
                Throw New Exception("Data Retrieval Error : " & exp.Message)
