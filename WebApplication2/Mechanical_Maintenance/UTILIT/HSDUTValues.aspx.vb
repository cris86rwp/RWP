...
#End Region
    ...
    '        rblArea.Visible = True
    '        RadioButtonList2.Visible = False

    '    Else
    '        rblArea.Visible = False
    ...
    Private Sub GetValue()
        ...
        'If RadioButtonList1.SelectedValue = "LPG" Then
        da.SelectCommand.Parameters.Add("@Item", SqlDbType.VarChar, 50).Value = rblArea.SelectedItem.Text.Trim
        'Else
        '    da.SelectCommand.Parameters.Add("@Item", SqlDbType.VarChar, 50).Value = RadioButtonList2.SelectedItem.Text.Trim
        'End If

        da.SelectCommand.CommandText = " select * from ms_HSDValues where  ItemDate = @dt  and Item = @Item ;"
        Try
            da.Fill(ds)
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        Try
            ...
            oCmd.Parameters.Add("@ItemType", SqlDbType.VarChar, 50).Value = RadioButtonList1.SelectedItem.Text
            'If RadioButtonList1.SelectedValue = "LPG" Then
            oCmd.Parameters.Add("@Item", SqlDbType.VarChar, 50).Value = rblArea.SelectedItem.Text
            'Else
            '    oCmd.Parameters.Add("@Item", SqlDbType.VarChar, 50).Value = RadioButtonList2.SelectedItem.Text
            'End If
            oCmd.Parameters.Add("@ChallanNumber", SqlDbType.Float, 8).Value = txtChallanNumber.Text
            oCmd.Parameters.Add("@ChallanDate", SqlDbType.Float, 8).Value = txtChallanDate.Text
            oCmd.Parameters.Add("@ItemValue", SqlDbType.Float, 8).Value = txtValue.Text
            oCmd.Parameters.Add("@Remarks", SqlDbType.VarChar, 50).Value = txtRemarks.Text.Trim
            ...
            If CheckBox1.Checked Then
                oCmd.CommandText = " select @SlNo = SlNo from ms_HSDValues where ItemDate = @ItemDate and  Item = @Item "
                oCmd.ExecuteScalar()
                If IIf(IsDBNull(oCmd.Parameters("@SlNo").Value), 0, oCmd.Parameters("@SlNo").Value) = 0 Then
                    oCmd.CommandText = "insert into ms_HSDValues " &
                           " ( ItemDate ,Item , ItemValue , Remarks,ChallanNumber, ChallanDate, Reset  ) " &
                                " values ( @ItemDate, @Item , @ItemValue , @Remarks,@ItemType ,@ChallanNumber, @ChallanDate, " & 1 & " )"

                Else
                    oCmd.CommandText = " update ms_HSDValues set ItemValue = @ItemValue , " &
                ...
                'If IIf(IsDBNull(oCmd.Parameters("@SlNo").Value), 0, oCmd.Parameters("@SlNo").Value) = 0 Then
                '    oCmd.CommandText = "insert into ms_HSDValues " &
                '                " ( ItemDate ,ItemType, Item , ItemValue ,ChallanNumber, ChallanDate, Remarks ) " &
                '                " values ( @ItemDate,@ItemType , @Item , @ItemValue ,@ChallanNumber, @ChallanDate, @Remarks )"
                'Else
                '    oCmd.CommandText = " update ms_HSDValues set ItemValue = @ItemValue , " &
