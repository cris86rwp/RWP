...
Public Class PrePouring
    ...
    Private Sub GetHeatDetails()
        ...
        Dim dt As New DataTable()
        Try
            dt = RWF.tables.GetPrePourHeatDetails(txtHeatNumber.Text)
            If dt.Rows.Count > 0 Then
                ...
                txtJMP_cover.DataValueField = CType(Session("PreNPost"), RWF.PreNPostPouring).JMPCover
                Try
                ...
                txtPour_tankNo.DataTextField = CType(Session("PreNPost"), RWF.PreNPostPouring).PourTankNo
                CType(Session("PreNPost"), RWF.PreNPostPouring).ALStars = IIf(IsDBNull(dt.Rows(0)("aluminium_stars")), "", dt.Rows(0)("aluminium_stars"))
                txtAL_stars.Text = CType(Session("PreNPost"), RWF.PreNPostPouring).ALStars
                ...
                txt_ladle_in_tank_temp.Text = CType(Session("PreNPost"), RWF.PreNPostPouring).LadleTemp
                '''
                If txt_ladle_in_tank_temp.Text = 0 Then txt_ladle_in_tank_temp.Text = ""
                ...
                '''
                Try
                    CType(Session("PreNPost"), RWF.PreNPostPouring).TubeInTime = CDate(dt.Rows(0)("tube1_in_time")).ToShortDateString + "  " + IIf(IsDBNull(dt.Rows(0)("tube1_in_time")), "00:00", Right(("0" + CDate(dt.Rows(0)("tube1_in_time")).Hour.ToString), 2) + ":" + Right(("0" + CDate(dt.Rows(0)("tube1_in_time")).Minute.ToString), 2))
                    txtTube_intime.Text = IIf(IsDBNull(dt.Rows(0)("tube1_in_time")), "00:00", Right(("0" + CDate(dt.Rows(0)("tube1_in_time")).Hour.ToString), 2) + ":" + Right(("0" + CDate(dt.Rows(0)("tube1_in_time")).Minute.ToString), 2))
                Catch exp As Exception
                    CType(Session("PreNPost"), RWF.PreNPostPouring).TubeInTime = CDate("01/01/1900").ToString + "  " + "00:00"
                    txtTube_intime.Text = "00:00"
                End Try
                '''
                If txtTube_intime.Text = "00:00" Then
                    txtTube_intime.Text = ""
                End If
                '''
                Try
                    CType(Session("PreNPost"), RWF.PreNPostPouring).TubeOutTime = CDate(dt.Rows(0)("tube1_out_time")).ToShortDateString + "  " + IIf(IsDBNull(dt.Rows(0)("tube1_out_time")), "00:00", Right(("0" + CDate(dt.Rows(0)("tube1_out_time")).Hour.ToString), 2) + ":" + Right(("0" + CDate(dt.Rows(0)("tube1_out_time")).Minute.ToString), 2))
                    txtTube_outtime.Text = IIf(IsDBNull(dt.Rows(0)("tube1_out_time")), "00:00", Right(("0" + CDate(dt.Rows(0)("tube1_out_time")).Hour.ToString), 2) + ":" + Right(("0" + CDate(dt.Rows(0)("tube1_out_time")).Minute.ToString), 2))
                Catch exp As Exception
                    CType(Session("PreNPost"), RWF.PreNPostPouring).TubeOutTime = CDate("01/01/1900").ToString + "  " + "00:00"
                    txtTube_outtime.Text = "00:00"
                End Try
                '''
                If txtTube_outtime.Text = "00:00" Then
                    txtTube_outtime.Text = ""
                End If
                '''
                CType(Session("PreNPost"), RWF.PreNPostPouring).LMLevel = IIf(IsDBNull(dt.Rows(0)("ladle_metal_level")), "", dt.Rows(0)("ladle_metal_level"))
                txtldlMtl_lvl.Text = CType(Session("PreNPost"), RWF.PreNPostPouring).LMLevel
                CType(Session("PreNPost"), RWF.PreNPostPouring).LIMetal = IIf(IsDBNull(dt.Rows(0)("ladle_insl")), "", dt.Rows(0)("ladle_insl"))
                txtldlInsl_metal.Text = CType(Session("PreNPost"), RWF.PreNPostPouring).LIMetal
                '''
                If txtldlInsl_metal.Text = 0 Then txtldlInsl_metal.Text = ""
                '''
                Try
                ...
                End Try
                Try
                    txtTITDate.Text = IIf(IsDBNull(dt.Rows(0)("tube1_in_time")), CDate(txtCastDate.Text), CDate(dt.Rows(0)("tube1_in_time")).ToShortDateString)
                Catch exp As Exception
                    txtTITDate.Text = CDate(txtCastDate.Text)
                End Try
                Try
                    txtTOTDate.Text = IIf(IsDBNull(dt.Rows(0)("tube1_out_time")), CDate(txtCastDate.Text), CDate(dt.Rows(0)("tube1_out_time")).ToShortDateString)
                Catch exp As Exception
                    txtTOTDate.Text = CDate(txtCastDate.Text)
                End Try
                i = Nothing
            Else
                txtLadlelift_time.Text = "00:00"
            ...
            If dt1.Rows.Count > 0 Then
                txtPourStartDate.Text = CDate(dt1.Rows(0)("PourStartTime")).Date
                txtPourStartTime.Text = IIf(IsDBNull(dt1.Rows(0)("PourStartTime")), "", Right(("0" + CDate(dt1.Rows(0)("PourStartTime")).Hour.ToString), 2) + ":" + Right(("0" + CDate(dt1.Rows(0)("PourStartTime")).Minute.ToString), 2))
    ...
    'Private Sub txtHeat_number_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
    '    lblMessage.Text = ""
    ...
    Public Function savepre()
        ...
        LMLevel = txtldlMtl_lvl.Text.Trim
        ...
        tplunser_pressure = Val(plgpressure.Text)
        ...
        Try
            ...
            cmd.CommandText = " insert into mm_pre_pouring ( cast_date , heat_number , furnace_code , " &
                    ...
                    '" & Val(txtRiser_weight.Text) & "', '" & txtTube1_mfg.Text & "','" & txtTube2_mfg.Text & "', '" & txtTube3_mfg.Text & "', 
		          '" & Val(txtTube1_life.Text) & "', '" & Val(txtTube2_life.Text) & "', '" & Val(txtTube3_life.Text) & "',
                    '" & txtPour_tankNo.SelectedItem.Value & "', '" & txtldlMtl_lvl.Text.Trim & "', '" & Val(txtldlInsl_metal.Text) & "', '" & Format(CDate(tbit), "MM/dd/yyyy HH:mm") & "' , getdate() , 
                    getdate() , '" & Format(CDate(tbot), "MM/dd/yyyy HH:mm") & "',getdate() , getdate() , getdate() , 
                    '" & Val(txt_ladle_in_tank_temp.Text) & "', '" & Val(txtMetalRecieved.Text) & "', '" & Val(txtOT13drag.Text) & "', '" & Val(txtOT14drag.Text) & "', '" & Val(txtOT20cope.Text) & "', '" & Val(txtOT21cope.Text) & "', 
                    getdate() ,'" & Val(plgpressure.Text) & "', '" & Val(prbheight.Text) & "')"
            ...
            '''        " ladle_temp , metal_recieved , D13, D14, C20, C21, pouring_start_time, plunser_pressure, " &
            '''        "probe_height ) " &
            '''        " values ( getdate() , @heat_number , @furnace_code , @operator_mould , @shift_supervisor , " &
            '''        " @jmp_cover1 ,getdate() ,getdate() , @tube1_immersion1_temperature , " &
            '''        "getdate(), @tube1_immersion2_temperature , getdate(), " &
            ...
            '''        " @ladle_insl ,getdate() ,getdate() ,getdate() ,getdate() , " &
            '''        "getdate() ,getdate() , "" , @ladle_temp ,  @metal_recieved , " &
            '''        " @D13, @D14, @C20, @C21, "", @plunser_pressure, @probe_height ) "
            'Else
            'cmd.CommandText = " update mm_pre_pouring set cast_date = @cast_date , furnace_code = @furnace_code , operator_mould = @operator_mould , " &
